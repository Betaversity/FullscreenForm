...
;( function( window ) {
	...
	jQuery(".mobileSelect").mobileSelect({
					title : "Select 5 tags",
					animation : "scale",
					theme : "holo",
				});	
	...
	FForm.prototype._saveDataToCookie = function(){
		// saveValue = [];
		var activeControls = jQuery('.fs-current .bv-data-source'); //get the item that's currently on screen 
		// for(x=0;x < activeControls.length;x++){
			var saveValue = jQuery(activeControls).val();
		// }
		var cookieKey = jQuery('.fs-current .bv-data-source').attr('id');
	...
	FForm.prototype._pullDataFromCookie = function(){
		...
		jQuery(".fs-current .bv-data-source").val(cookieData.responseValue);
	}
